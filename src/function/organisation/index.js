...
export const insertOwner = (data) => {
    ...
    return new Promise((resolve, reject) => {
        try {
            execute(
                'INSERT INTO owner(id, name, user_id, address, city, state, country, phone_no, fax, email, name_person, phone1, phone2, fax_contact, email_contact, department, position, comment, ref_id, mode)' +
                ' VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
                [
