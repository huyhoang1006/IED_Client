...
export const getAnalogById = async (mrid) => {
    try {
        ...
        return new Promise((resolve, reject) => {
            db.get(`SELECT * FROM analog WHERE mrid=?`, [mrid], (err, row) => {
...
// Lấy tất cả analog theo procedure_id
    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            dbsql.run(
                ...
                `,
                [analog.mrid, analog.max_value, analog.min_value, analog.normal_value, analog.positive_flow_in],
                function (err) {
        ...
        try {
            ...
            dbsql.run(
                `UPDATE analog SET
                    ...
                    positive_flow_in = ?
                WHERE mrid = ?`,
                [analog.max_value, analog.min_value, analog.normal_value, analog.positive_flow_in, mrid],
                function (err) {
            ...
            db.run('BEGIN TRANSACTION', async (err) => {
                ...
                try {
                    ...
                    db.run('DELETE FROM analog WHERE mrid=?', [mrid], function (err) {
                        ...
                        // Xóa measurement sau khi xóa analog
                        measurementFunc
                            .deleteMeasurementByIdTransaction(mrid, db)
                            .then(() => {
                                db.run('COMMIT', (err) => {
                                    if (err) {
                                        return reject({success: false, err, message: 'Commit failed'})
