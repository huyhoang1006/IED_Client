// File: src/function/cim/parentOrganization/index.js
...
export const insertParentOrganisation = (parentOrganization) => {
    return new Promise((resolve, reject) => {
        console.log('Inserting parent organization:', parentOrganization);
        db.serialize(async () => {
            // Disable foreign key checks ngay từ đầu
            db.run('PRAGMA foreign_keys = OFF');
            db.run('BEGIN TRANSACTION');
            try {
                console.log('Calling insertOrganisationTransaction...');
                const orgResult = await organisationFunc.insertOrganisationTransaction(parentOrganization, db);
                console.log('Organisation insert result:', orgResult);
                ...
                console.log('Inserting into parent_organization table...');
                db.run(
                    `INSERT INTO parent_organization(mrid) VALUES (?) ON CONFLICT(mrid) DO NOTHING`,
                    ...
                    function(err) {
                        ...
                        console.log('Successfully inserted into parent_organization');
                        db.run('COMMIT');
