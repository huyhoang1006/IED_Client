...
export const getStreetDetailByLocationId = async (locationId) => {
    return new Promise((resolve, reject) => {
        const query = `
            SELECT sd.* 
            FROM location l
            JOIN street_address sa ON l.main_address = sa.mrid
            JOIN street_detail sd ON sa.street_detail = sd.mrid
            WHERE l.mrid = ?
        `;

        db.get(query, [locationId], (err, row) => {
            if (err) {
                return reject({ success: false, err: err, message: 'Get street detail by location id failed' });
            }
            if (!row) {
                return resolve({ success: false, data: null, message: 'Street detail not found' });
            }
            return resolve({ success: true, data: row, message: 'Get street detail by location id completed' });
        });
    });
};
        ...
        db.run(
            ...
            number, prefix, suffix, suite_number, type, within_town_limits)
             ...
             ON CONFLICT(mrid) DO UPDATE SET
                ...
                code = excluded.code,
                floor_identification = excluded.floor_identification,
...
export const insertStreetDetailTransaction = async (streetDetail, dbsql) => {
    return new Promise((resolve, reject) => {
        dbsql.run(
            `INSERT INTO street_detail(mrid, address_general, building_name, code, 
            floor_identification, number, prefix, suffix, suite_number, type, within_town_limits)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
             ON CONFLICT(mrid) DO UPDATE SET
                address_general = excluded.address_general,
                building_name = excluded.building_name,
    ...
    return new Promise((resolve, reject) => {
        db.run(
            `UPDATE street_detail
             SET address_general = ?, building_name = ?, code = ?, floor_identification = ?, number = ?,
                 prefix = ?, suffix = ?, suite_number = ?, type = ?, within_town_limits = ?
             WHERE mrid = ?`,
            [streetDetail.address_general, streetDetail.building_name, streetDetail.code, streetDetail.floor_identification,
                streetDetail.number, streetDetail.prefix, streetDetail.suffix, streetDetail.suite_number, streetDetail.type, streetDetail.within_town_limits, mrid],
            function (err) {
...
export const updateStreetDetailByIdTransaction = async (mrid, streetDetail, dbsql) => {
    return new Promise((resolve, reject) => {
        dbsql.run(
            `UPDATE street_detail
             SET address_general = ?, building_name = ?, code = ?, floor_identification = ?,
                 number = ?, prefix = ?, suffix = ?, suite_number = ?, type = ?, within_town_limits = ?
             WHERE mrid = ?`,
            [streetDetail.address_general, streetDetail.building_name, streetDetail.code, streetDetail.floor_identification,
                streetDetail.number, streetDetail.prefix, streetDetail.suffix, streetDetail.suite_number, streetDetail.type, streetDetail.within_town_limits, mrid],
