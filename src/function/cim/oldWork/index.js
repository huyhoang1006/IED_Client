...
import * as workFunc from '../work/index.js'
...
export const deleteOldWorkByIdTransaction = async (mrid, dbsql) => {
    return new Promise(async (resolve, reject) => {
        try {
            dbsql.run("DELETE FROM old_work WHERE mrid=?", [mrid], async function (err) {
                ...
                try {
                    await workFunc.deleteWorkByIdTransaction(mrid, dbsql)
