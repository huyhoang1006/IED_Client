    ...
    return new Promise((resolve, reject) => {
        powerSystemResourceFunc.insertPowerSystemResourceTransaction(cnc, dbsql)
            .then(result => {
                ...
                // Verify mrid exists in power_system_resource before inserting into connectivity_node_container
                // This ensures the foreign key constraint is satisfied
                dbsql.get('SELECT mrid FROM power_system_resource WHERE mrid = ?', [cnc.mrid], (verifyErr, verifyRow) => {
                    if (verifyErr) {
                        return reject({ success: false, err: verifyErr, message: 'Failed to verify PowerSystemResource exists' })
                    }
                    if (!verifyRow) {
                        return reject({ 
                            success: false, 
                            err: new Error('PowerSystemResource not found'), 
                            message: `Cannot insert ConnectivityNodeContainer: mrid '${cnc.mrid}' does not exist in power_system_resource table` 
                        })
                    }
                    
                    // Now safe to insert into connectivity_node_container
                    dbsql.run(
                        `INSERT INTO connectivity_node_container(mrid)
                         VALUES (?)
                         ON CONFLICT(mrid) DO NOTHING`,
