...
export const insertGeoMapArrayTransaction = async (geoMaps, dbsql) => {
    return new Promise((resolve, reject) => {
        ...
        validGeoMaps.forEach((geoMap, index) => {
            if (hasError) return
                ...
                function (err) {
                    if (err) {
                        ...
                        return reject({
                            success: false,
                            err: errors,
                            message: `Insert geo map array failed at index ${index}: ${err.message}`
                        })
                    }

                    completedCount++
                    if (completedCount === validGeoMaps.length) {
                        return resolve({
                            success: true,
                            data: validGeoMaps,
                            message: `Insert ${validGeoMaps.length} geo maps completed`
                        })
                    }
                }
            )
...
// Update geoMap array by id transaction
...
// Delete geoMap by array mrid transaction
