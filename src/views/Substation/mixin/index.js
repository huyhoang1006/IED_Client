            ...
            if (this.properties.name === '') {
                ...
                if (!sign) {
                    return {
                        ...
                        try {
                            // Validate dto has subsId before mapping
                            if (!dto.subsId) {
                                this.$message.error('Substation ID is required but is missing')
                                return {
                                    success: false,
                            ...
                            if (!substationEntity.substation || !substationEntity.substation.mrid) {
                                this.$message.error('Substation ID is missing after mapping')
                                return {
                                    success: false,
                                    error: 'Substation ID is missing after mapping'
                                }
                            }
                            ...
                            if (result.success) {
                                ...
                                const errorDetails = result?.error ? (typeof result.error === 'string' ? result.error : JSON.stringify(result.error, null, 2)) : ''
                    ...
                    this.isSaving = false; // Reset flag on outer error
                    throw error;
                }
            }
        },
