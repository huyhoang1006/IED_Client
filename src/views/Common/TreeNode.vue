    ...
    <li>
        <span @contextmenu.prevent="openContextMenu($event, node)" :class="{ selected: selectedNodes.some(n => n.id === node.id) }" class="folder" @click="toggle" @dblclick="doubleToggle">
            ...
            <div v-else-if="node.mode == 'asset'" class="icon-wrapper">
                <icon size="16px" folderType="asset" :assetDetail="node.asset || 'Unknown'" badgeColor="146EBE"></icon>
...
export default {
    ...
    methods: {
        async toggle(event) {
            ...
            this.clickTimeout = setTimeout(() => {
                if(event.ctrlKey) {
                    ...
                    // Chỉ expand/collapse khi node có children hoặc có thể fetch children
                    if (this.hasChildrenOrCanFetch()) {
        ...
        doubleToggle(event) {
            ...
            // Double click là action chính - luôn thực hiện
            // Không cần check children, action chính sẽ được xử lý ở component cha
            this.$emit("double-click-node", this.node);
        },

        hasChildrenOrCanFetch() {
            ...
            return this.node.children && this.node.children.length > 0 || 
                   canHaveChildren.includes(this.node.mode);
        },
