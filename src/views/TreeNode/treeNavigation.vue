<template>
    <div class="explorer">
                ...
                <div class="child-nav">
                    <ul>
                        <TreeNode v-for="item in organisationClientList" :key="item.id" :node="item"
                            @double-click-node="doubleClickNode" v-model:selectedNodes="selectedNodes"
            ...
            <div ref="sidebarServer" v-show="!clientSlide" class="sidebar">
                ...
                <div class="child-nav">
                    <ul>
                        <TreeNode v-for="item in ownerServerList" :key="item.id" :node="item"
                            v-model:selectedNodes="selectedNodes" @fetch-children="fetchChildrenServer"
                ...
                <div class="page-align">
                    <page-align ref="LocationSyncPageAlign" :page-user="this.pageLocationSync"
                        ...
                        title="LocationSync" v-model:option="this.optionLocationSync"
        ...
        <el-dialog title="Add Job" v-model:visible="signJob" width="1000px" @close="handleJobCancel">
     ...
     methods: {
        ...
        handleDropdown() {
            ...
            if (this.$refs.dropdown) {
                // Element UI dropdown doesn't have handleClick method
                // Instead, we can trigger the dropdown programmatically
        ...
        resizeClient(event) {
            ...
            const finalWidth = Math.max(10, Math.min(40, newWidth));
        ...
        async fetchChildren(node) {
            if (!node.children) {
                try {
                    ...
                    if (node.mode == 'asset') {
                        ...
                        if (organisationReturn.success && organisationReturn.data && organisationReturn.data.length > 0) {
                            ...
                            organisationReturn.data.forEach(row => {
                                ...
                                const parentName = clickedRow.parentName + "/" + clickedRow.name
                                row.parentName = parentName
                                row.parentArr = this.buildParentArr(clickedRow, row)
                            });
                    ...
                    node.children = newRows; // Vue 3 reactivity
                } catch (error) {
        ...
        async showPropertiesData(node) {
            ...
            if (node.asset !== undefined) {
                ...
                // Thêm node hiện tại vào pathMapServer
        ...
        async mappingProperties(data) {
            if (data !== undefined) {
                ...
                this.properties.region = data.region || ''
                this.properties.address = data.address || ''
                this.properties.city = data.city || ''
                this.properties.state_province = data.state_province || ''
                this.properties.postal_code = data.postal_code || ''
                this.properties.country = data.country || ''
                this.properties.phone_no = data.phone_no || ''
                this.properties.email = data.email || ''
            }
        },

        async mappingAssetProperties(data) {
            if (data !== undefined) {
                this.assetProperties.asset = data.asset || ''
                this.assetProperties.asset_type = data.asset_type || ''
                this.assetProperties.serial_no = data.serial_no || ''
                this.assetProperties.manufacturer = data.manufacturer || ''
                this.assetProperties.manufacturer_type = data.manufacturer_type || ''
                this.assetProperties.manufacturing_year = data.manufacturing_year || ''
                this.assetProperties.country = data.country || ''
                this.assetProperties.apparatus_id = data.apparatus_id || ''
            }
        },

        async mappingJobProperties(data) {
            if (data !== undefined) {
                this.jobProperties.name = data.name || ''
                this.jobProperties.work_order = data.work_order || ''
                this.jobProperties.creation_date = data.creation_date || ''
                this.jobProperties.execution_date = data.execution_date || ''
                this.jobProperties.approved_by = data.approved_by || ''
                this.jobProperties.ambient_condition = data.ambient_condition || ''
                this.jobProperties.tested_by = data.tested_by || ''
                this.jobProperties.standard = data.standard || ''
            }
        },

        async getOwnerLocation() {
            try {
                // Mock data for now - replace when demoAPI is available
                const res = { mrid: 'mock-owner', name: 'Mock Owner', mode: 'organisation' };
                if (res !== null) {
            ...
            try {
                ...
                if (org && org.saveOrganisation) {
                    ...
                    if (success) {
                        ...
                        this.signOrg = false
                        if (org.properties) {
                            // Đảm bảo parentId được set đúng - nếu không có parent thì dùng ROOT_ID
                            const ROOT_ID = this.$constant?.ROOT || '00000000-0000-0000-0000-000000000000'
                            const parentId = org.properties.parentId || this.parentOrganization?.mrid || ROOT_ID
                            const savedNode = {
                                mrid: org.properties.organisationId,
                                id: org.properties.organisationId,
                                name: org.properties.name,
                                mode: 'organisation',
                                parentId: parentId,
                                parentName: this.parentOrganization?.name || (parentId === ROOT_ID ? 'Root' : ''),
                                parentArr: this.parentOrganization?.parentArr || []
                            }
                            this.addOrganisationToTree(savedNode)
                        }
